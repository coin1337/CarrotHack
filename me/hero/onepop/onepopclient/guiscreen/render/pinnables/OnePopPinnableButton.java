...
public class OnePopPinnableButton {
   private OnePopPinnable pinnable;
   ...
   public OnePopPinnableButton(OnePopFrame master, String name, String tag) {
      this.master = master;
      this.name = name;
      this.tag = tag;
      this.pinnable = OnePop.get_hud_manager().get_pinnable_with_tag(tag);
      this.x = master.get_x();
      this.y = master.get_y();
      this.save_y = this.y;
   ...
   public void render(int mx, int my, int separate) {
      ...
      if (this.pinnable.is_active()) {
         OnePopDraw.draw_rect(this.x, this.save_y, this.x + this.width - separate, this.save_y + this.height, bg_r, bg_g, bg_b, bg_a);
         OnePopDraw.draw_string(this.pinnable.get_title(), this.x + separate, this.save_y, nc_r, nc_g, nc_b, nc_a);
      } else {
         OnePopDraw.draw_string(this.pinnable.get_title(), this.x + separate, this.save_y, nc_r, nc_g, nc_b, nc_a);
      }
