...
public class OnePopPinnable {
   private String title;
   private String tag;
   private boolean state;
   private boolean move;
   public OnePopDraw font;
   private int x;
   private int y;
   private int width;
   private int height;
   ...
   public OnePopPinnable(String title, String tag, float font_, int x, int y) {
      this.title = title;
      this.tag = tag;
      this.font = new OnePopDraw(font_);
      this.x = x;
      this.y = y;
      this.width = 1;
      this.height = 10;
   ...
   public void render(int mx, int my, int tick) {
      ...
      if (this.is_active()) {
         ...
         if (this.motion(mx, my)) {
            OnePopDraw.draw_rect(this.x - 1, this.y - 1, this.width + 1, this.height + 1, 0, 0, 0, 90, 2, "right-left-down-up");
         }
      }
   ...
   protected void create_line(String string, int pos_x, int pos_y) {
      OnePopDraw.draw_string(string, this.x + pos_x, this.y + pos_y, 255, 255, 255, 255);
   }

   protected void create_line(String string, int pos_x, int pos_y, int r, int g, int b, int a) {
      OnePopDraw.draw_string(string, this.x + pos_x, this.y + pos_y, r, g, b, a);
   }

   protected void create_rect(int pos_x, int pos_y, int width, int height, int r, int g, int b, int a) {
      OnePopDraw.draw_rect(this.x + pos_x, this.y + pos_y, this.x + width, this.y + height, r, g, b, a);
   }
   ...
   protected boolean is_on_gui() {
      return OnePop.click_hud.on_gui;
   }
}
