package me.hero.onepop.onepopclient.guiscreen.render.pinnables;
...
public class OnePopFrame {
   private ArrayList<OnePopPinnableButton> pinnable_button = new ArrayList();
   private String name;
   private String tag;
   private int x;
   private int y;
   ...
   public OnePopFrame(String name, String tag, int initial_x, int initial_y) {
      this.name = name;
      this.tag = tag;
      this.x = initial_x;
      this.y = initial_y;
      ...
      this.height = 25;
      this.can = true;
      int size = OnePop.get_hud_manager().get_array_huds().size();
      int count = 0;
      Iterator var7 = OnePop.get_hud_manager().get_array_huds().iterator();

      while(var7.hasNext()) {
         OnePopPinnable pinnables = (OnePopPinnable)var7.next();
         OnePopPinnableButton pinnables_buttons = new OnePopPinnableButton(this, pinnables.get_title(), pinnables.get_tag());
   ...
   public void mouse(int mx, int my, int mouse) {
      ...
      while(var4.hasNext()) {
         OnePopPinnableButton pinnables_buttons = (OnePopPinnableButton)var4.next();
   ...
   public void render(int mx, int my, int separate) {
      ...
      OnePopDraw.draw_rect(this.x - 1, this.y, this.width + 1, this.height, bd_r, bd_g, bd_b, bd_a, this.border_size, "left-right");
      OnePopDraw.draw_string(this.name, this.x + 4, this.y + 4, nc_r, nc_g, nc_b, nc_a);
      if (this.is_moving()) {
         this.crush(mx, my);
      }
      ...
      while(var7.hasNext()) {
         OnePopPinnableButton pinnables_buttons = (OnePopPinnableButton)var7.next();
         pinnables_buttons.set_x(this.x + separate);
         pinnables_buttons.render(mx, my, separate);
         if (pinnables_buttons.motion(mx, my)) {
            OnePopDraw.draw_rect(this.get_x() - 1, pinnables_buttons.get_save_y(), this.get_width() + 1, pinnables_buttons.get_height(), bdw_r, bdw_g, bdw_b, bdw_a, this.border_size, "right-left");
         }
      }
