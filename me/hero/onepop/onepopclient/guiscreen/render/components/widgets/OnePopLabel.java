...
public class OnePopLabel extends OnePopAbstractWidget {
   ...
   private final OnePopDraw font = new OnePopDraw(1.0F);
   private int border_size = 0;

   public OnePopLabel(OnePopFrame frame, OnePopModuleButton master, String tag, int update_postion) {
      this.frame = frame;
      this.master = master;
   ...
   public void render(int master_y, int separe, int absolute_x, int absolute_y) {
      ...
      if (this.info) {
         OnePopDraw.draw_string(this.setting.get_value(s), this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
      } else {
         OnePopDraw.draw_string(this.label_name + " \"" + this.setting.get_value(s) + "\"", this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
      }
