...
import me.hero.turok.values.TurokOldDouble;

public class OnePopSlider extends OnePopAbstractWidget {
   ...
   public OnePopSlider(OnePopFrame frame, OnePopModuleButton master, String tag, int update_postion) {
      this.frame = frame;
      this.master = master;
   ...
   public void render(int master_y, int separe, int absolute_x, int absolute_y) {
      ...
      if (this.click) {
         if (mouse != 0.0D) {
            this.setting.set_value(TurokOldDouble.round(mouse / (double)this.width * (this.setting.get_max(1.0D) - this.setting.get_min(1.0D)) + this.setting.get_min(1.0D)));
         } else {
      ...
      OnePopDraw.draw_rect(this.x, this.save_y, this.x + this.width * (this.setting.get_value(1) - this.setting.get_min(1)) / (this.setting.get_max(1) - this.setting.get_min(1)), this.save_y + this.height, bg_r, bg_g, bg_b, bg_a);
      OnePopDraw.draw_string(this.slider_name, this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
      OnePopDraw.draw_string(slider_value, this.x + this.width - separe - this.font.get_string_width(slider_value) + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
   }
}
